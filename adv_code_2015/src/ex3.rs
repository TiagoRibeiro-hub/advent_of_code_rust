...
pub fn read_input_three() -> String {
    std::fs::read_to_string("/home/tiago/rust/projects/advent_of_code/advent_of_code/files/grid_of_houses.txt")
...
fn x_func_hash_set(points: &mut HashMap<i32, HashSet<i32>>, last_y: i32, last_x: i32, sum: &mut i32) {
    if let Some(value) = points.get_mut(&last_y) {
        if !value.contains(&last_x) {
            value.insert(last_x);
            *sum+= 1;
...
fn y_func_hash_set(points: &mut HashMap<i32, HashSet<i32>>, last_y: i32, last_x: i32, sum: &mut i32) {
    if !points.contains_key(&last_y) {
        points.insert(last_y, HashSet::from([last_x]));
        *sum+= 1;
    }
